...
FormDD primary host: `https://formdd.xambrain.com/releases/latest.json`  
...
1. A build produces under `dist/installer/`:
   - `FormDD-Setup-x.y.z.exe` (never a generic `FormDD-Setup.exe` that gets overwritten)
   - `latest.json` (includes `sha256` / `size`)
   - `SHA256SUMS.txt`
2. Upload the **new** `.exe` to `/var/www/formdd/releases/` — **do not overwrite** an older `FormDD-Setup-x.y.z.exe`
...
{
  "version": "0.3.1",
  "setup_url": "https://formdd.xambrain.com/releases/FormDD-Setup-0.3.1.exe",
...
- `setup_url` must be HTTPS and the filename must match the version, e.g. `FormDD-Setup-0.3.1.exe`
...
```text
FormDD-Setup-0.3.0.exe
FormDD-Setup-0.3.1.exe
FormDD-Setup-0.4.0.exe
```

Incorrect: `FormDD-Setup.exe` or `PDFFormMarker-Setup.exe` overwritten on every release.
...
Internal repo / install folder names may stay `PDFFormMarker` (`PDFFormMarker.exe`, `%LOCALAPPDATA%\PDFFormMarker`). The customer-facing download is **FormDD**.
...
/releases/FormDD-Setup-0.3.0.exe
Cache-Control: public, max-age=31536000, immutable
```

`latest.json` changes often; a versioned `.exe` must never change.  
Example config: `website/Caddyfile.formdd`

The app fetches the feed with `Cache-Control: no-cache` and a ~3s timeout.

## GitHub Releases (every time — backup, not primary host)

The marketing/update host remains `formdd.xambrain.com`.  
Still create a GitHub Release for every version, e.g. tag `v0.3.1` titled **FormDD v0.3.1**.
...
gh release create v0.3.1 `
  dist/installer/FormDD-Setup-0.3.1.exe `
  dist/installer/SHA256SUMS.txt `
  --title "FormDD v0.3.1" `
  --notes-file CHANGELOG.md
```

Do not point `latest.json` at GitHub by default — keep `formdd.xambrain.com`.

## Secrets must not live on the public site

`/var/www/formdd` may contain: HTML, CSS, JS, installer, `latest.json`.

Never place: `private.key`, `license-master.key`, `gen_license.py`, `.env`, a customer database.

A future online Activate belongs on `api.formdd.xambrain.com` (or `/api` via reverse proxy).  
