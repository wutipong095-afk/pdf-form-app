...
## Third-party notices (added 2026-09-08)

`THIRD_PARTY_NOTICES.txt` now aggregates the license text of every
third-party component shipped in the installed application, and is bundled
into the app (see `PDFFormMarker.spec` `datas`). It is regenerated by
`scripts/collect_notices.py`, which harvests license text from:

* installed Python distribution metadata (Flask, Werkzeug, Jinja2, click,
  itsdangerous, MarkupSafe, blinker, waitress, cryptography, cffi,
  pycparser);
* the vendored `third_party_licenses/` tree (pypdf, pypdfium2 + PDFium
  build deps, ReportLab + Bitstream Vera, uharfbuzz, fontTools, Pillow);
* hand-written notices under `notices/` for items with no machine-readable
  license file: the fonts (Noto Sans Thai under OFL-1.1, TH Sarabun New and
  TH Sarabun IT๙ as free Thai national fonts — usage rights verified by the
  owner) and the OpenSSL build statically linked by `cryptography`.

The generator fails loudly if any declared component yields no license
text, so the notices file cannot silently ship incomplete. Run it inside an
environment with the runtime dependencies installed before each release:
`python scripts/collect_notices.py`.

This closes the notice-completeness gaps flagged below. Remaining before a
public sale: independent legal sign-off and the sales EULA (see the note at
the end of "Decision recorded here").
