...
ออกคีย์: `python scripts/gen_license.py <รหัส16ตัว> --term 1` หรือ `--days 36500` (ซื้อขาด)  
ไทย: 1 ปี `--term 1` · ซื้อขาด / ซื้อขาด+อัปเดต 3 ปี `--days 36500`  
ต่างประเทศ `--term 10` (อย่าเขียนในอีเมลว่า 10 ปี — บอกจ่ายครั้งเดียว อัปเดตตราบที่คีย์ยังใช้ได้ วันหมดอายุอยู่ที่แถบไลเซนต์)  
ย้ายเครื่องที่เหลือวันใช้ `--days N` แทน `--term` (วันหมดอายุ UTC ใช้ `--days 0`)  
ส่งคีย์ย้ายเครื่องด้วยบล็อก «ไทย — ส่งคีย์ (ย้ายเครื่อง)» ไม่ใช่บล็อกขายใหม่
...
ลอก `{EXPIRES}` จากบรรทัดที่สคริปต์พิมพ์ (`หมดอายุ YYYY-MM-DD UTC`) — อย่าคำนวณเอง
...
## ไทย — ส่งคีย์ (ย้ายเครื่อง)

เรื่อง: FormDD — คีย์เครื่องใหม่ {MACHINE_ID}

ลอก `{EXPIRES}` จากบรรทัดที่สคริปต์พิมพ์ (`หมดอายุ YYYY-MM-DD UTC`) — อย่าคำนวณเอง
ใช้บล็อกนี้เมื่อออกด้วย `--days` ไม่ใช่ `--term`

```
คีย์สำหรับเครื่องใหม่ {MACHINE_ID}:

{KEY}

ใช้ได้ถึง {EXPIRES} (UTC)
คีย์นี้ยกวันที่เหลือจากใบเดิมมา ไม่ใช่อายุใหม่
